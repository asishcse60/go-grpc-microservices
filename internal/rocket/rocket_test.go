...
func TestRocketService(t *testing.T){
	...
	t.Run("test delte rocket", func(t *testing.T) {
		rocketMockStore := NewMockStore(mockCtrl)
		id := "UUID-1"
		rocketMockStore.EXPECT().DeleteRocket(id).Return(nil)
		rocketService := New(rocketMockStore)
